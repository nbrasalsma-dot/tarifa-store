...
export default function ProductDetailPage() {
  ...
  const handleSubmitReview = async () => {
    if (!user) {
      toast({ title: "تنبيه", description: "يرجى تسجيل الدخول أولاً للتقييم", variant: "destructive" });
      setIsAuthOpen(true);
      return;
    }
    ...
    try {
      const response = await fetch("/api/reviews", {
        method: "POST",
        body: JSON.stringify({ productId, userId: user.id, rating: userRating, comment: userComment }),
      });
      if (response.ok) {
  ...
  const handleAddToCart = () => {
    ...
    if (product.colors) {
      try {
        availableColors = typeof product.colors === "string" 
 ...
 // --- مترجم الألوان (لتحويل الاسم العربي إلى شكل لوني) ---
  ...
  const productColors = useMemo(() => {
    ...
    try {
      let parsedData = product.colors;

      if (typeof product.colors === "string") {
        if (product.colors.startsWith("[")) {
          parsedData = JSON.parse(product.colors);
        } else {
          parsedData = product.colors.split(",").map(c => c.trim()).filter(c => c !== "");
        }
      }
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            >
              ...
              {/* Title */}
              <div>
                <h1 className="text-3xl md:text-4xl font-bold mb-2 text-[#3D3021]">{product.nameAr}</h1>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  ></span>
                  <span className={`font-bold ${product.stock > 0 ? "text-green-600" : "text-orange-600"}`}>
                    {product.stock > 0 ? `متوفر في المخزن (${product.stock} قطعة)` : "سيتم طلبه من أجلك (غير متوفر بالمخزن)"}
                  </span>
                </div>
              ...
              {product.descriptionAr && (
                <div>
                  ...
                  <p className="text-[#5D5D5D] leading-relaxed">{product.descriptionAr}</p>
                </div>
              )}
                  ...
                  <div className="flex flex-wrap gap-3">
                    {productColors.map((colorItem: any, index: number) => (
                      ...
                      >
                        ...
                        <span className="flex-1 text-right">{colorItem.label}</span>
                        {selectedColor === colorItem.label && (
              ...
              {productSizes && productSizes.length > 0 && (
                <div className="space-y-3 mb-6">
                  <h3 className="font-bold text-[#3D3021]">المقاس / الحجم المتوفر:</h3>
                  ...
                  {selectedSize && (
                    <p className="text-xs text-[#C9A962] font-medium">المقاس المختار: {selectedSize}</p>
                  )}
                </div>
              )}
              {/* Quantity Selector */}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Button variant="outline" size="lg" className="py-5 rounded-full border-[#C9A962] text-[#8B7355]" onClick={handleShare}>
              ...
              {product.videoUrl && (
                <div className="space-y-3 pt-6 border-t border-[#E8E0D8] mt-6">
                  <h3 className="font-bold text-[#3D3021] flex items-center gap-2">
                    <span className="text-pink-500">📷</span> شاهد المنتج على الطبيعة
                  </h3>
                  ...
                  <p className="text-[10px] text-center text-gray-400">إذا لم يظهر الفيديو، تأكد أن الحساب "عام" وليس "خاص"</p>
                </div>
              )}
               {/* تفاصيل المنتج الإضافية (الأكورديون الفخم) */}
              {(product.featuresAr || product.usageAr || product.ingredientsAr) && (
                <div className="pt-6 border-t border-[#E8E0D8] mt-6">
                  <Accordion type="single" collapsible className="w-full">
                    {product.featuresAr && (
                      <AccordionItem value="features" className="border-b border-[#E8E0D8]">
                        <AccordionTrigger className="text-[#3D3021] font-bold hover:text-[#C9A962] transition-colors">
                          <span className="flex items-center gap-2 text-base">✨ المميزات والفوائد</span>
                        </AccordionTrigger>
                    ...
                    {product.usageAr && (
                      <AccordionItem value="usage" className="border-b border-[#E8E0D8]">
                        <AccordionTrigger className="text-[#3D3021] font-bold hover:text-[#C9A962] transition-colors">
                          <span className="flex items-center gap-2 text-base">📝 طريقة الاستخدام</span>
                        </AccordionTrigger>
                        <AccordionContent className="text-[#5D5D5D] leading-relaxed text-sm whitespace-pre-line py-4 px-2 bg-white/30 rounded-lg">
                          {product.usageAr}
                        </AccordionContent>
                    ...
                    {product.ingredientsAr && (
                      <AccordionItem value="ingredients" className="border-0">
                        <AccordionTrigger className="text-[#3D3021] font-bold hover:text-[#C9A962] transition-colors">
                          <span className="flex items-center gap-2 text-base">🧪 المكونات</span>
                        </AccordionTrigger>
                        <AccordionContent className="text-[#5D5D5D] leading-relaxed text-sm whitespace-pre-line py-4 px-2 bg-white/30 rounded-lg">
                          {product.ingredientsAr}
                        </AccordionContent>
                ...
                <div className="pt-10 border-t border-[#E8E0D8] mt-10 space-y-8">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <h2 className="text-2xl font-bold text-[#3D3021]">آراء العملاء ⭐</h2>
                      <p className="text-xs text-[#8B7355]">ماذا يقول جيراننا عن هذا المنتج</p>
                    </div>
                    <Badge variant="outline" className="text-[#C9A962] border-[#C9A962] px-4 py-1.5 bg-white shadow-sm font-bold">
                  ...
                  <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                    {reviews.length > 0 ? (
                        ...
                        >
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-bold text-[#3D3021] text-sm">{rev.user?.name || "عميل ترفة"}</p>
                                <div className="flex items-center gap-2">
                                   <div className="flex text-yellow-400">
                                    {[...Array(5)].map((_, i) => (
                                      <Star key={i} className={`h-3 w-3 ${i < rev.rating ? "fill-yellow-400" : "text-gray-200"}`} />
                                    ))}
                                  </div>
                                  <span className="text-[9px] text-gray-400">
                                    {new Date(rev.createdAt).toLocaleDateString('ar-YE', { day: 'numeric', month: 'long' })}
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              ...
                              {(user?.role === 'ADMIN' || user?.id === (product as any).merchantId || user?.id === (product as any).agentId) && (
                                <Button
                                  variant="ghost"
                                  size="icon"
                      ...
                      <div className="text-center py-16 bg-white/50 rounded-3xl border-2 border-dashed border-[#E8E0D8] space-y-3">
                        ...
                        <div className="space-y-1">
                           <p className="text-[#3D3021] font-medium">لا توجد تقييمات حتى الآن</p>
                           <p className="text-xs text-gray-400">كن أول من يشاركنا تجربته ويساعد الآخرين</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-[#FAF7F2] to-[#F5EFE6] p-8 rounded-[2.5rem] border border-[#E8E0D8] shadow-sm space-y-6 relative overflow-hidden">
                    <div className="relative z-10 space-y-6">
                      <div className="space-y-2 text-center md:text-right">
                        <h3 className="font-bold text-xl text-[#3D3021]">اترك بصمتك ✨</h3>
                        <p className="text-xs text-[#8B7355]">رأيك يهمنا ويساعدنا على تقديم الأفضل دائماً</p>
                      </div>
      
                      <div className="flex flex-col items-center md:items-start gap-4">
                        <p className="text-sm font-medium text-[#3D3021]">كيف تقيم تجربتك؟</p>
                            ...
                            >
                              <Star 
                                className={`h-8 w-8 transition-colors ${
                                  userRating >= star ? "fill-yellow-400 text-yellow-400 shadow-yellow-200" : "text-gray-300"
                                }`} 
                              />
                            </button>
                          ))}
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center px-1">
                           <p className="text-sm font-medium text-[#3D3021]">تعليقك الصادق:</p>
                      ...
                      <div className="space-y-4">
                        <Button 
                          ...
                          disabled={isSubmittingReview || (user && !userComment.trim())}
                        ...
                        {!user && (
                          <div className="flex items-center gap-2 justify-center bg-rose-50 p-3 rounded-xl border border-rose-100">
                            ...
                            <p className="text-[11px] text-rose-600 font-medium italic">
                              عزيزي الزائر، التقييم متاح فقط للعملاء المسجلين لضمان جودة ومصداقية التعليقات.
                            </p>
                          </div>
                        )}
                      </div>
          ...
          {relatedProducts.length > 0 && (
            <section className="mt-16">
              <h2 className="text-2xl font-bold mb-6 text-[#3D3021]">منتجات مشابهة</h2>
                  ...
                  <Link key={item.id} href={`/products/${item.id}`}>
                    <Card className="overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all cursor-pointer bg-white rounded-2xl">
                      ...
                      <CardContent className="p-4">
                        ...
                        <h3 className="font-semibold text-sm text-[#3D3021] line-clamp-1">{item.nameAr}</h3>
                        <p className="font-bold text-[#8B7355]">{formatPrice(item.price)}</p>
                      </CardContent>
                    </Card>
                  </Link>
                ))}
