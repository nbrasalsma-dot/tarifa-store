...
export async function POST(req: Request) {
    try {
        ...
        const { fullName, email, phone, password, role, storeName, storeType, address } = body;
        ...
        if (existingUser) {
            return NextResponse.json({ success: false, error: "البريد أو الهاتف مستخدم مسبقاً" }, { status: 400 });
        }
