...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("📝 [Contacts API] - Base whereClause roles:", [
      "ADMIN",
      "AGENT",
      "MERCHANT",
    ]);
    ...
    // 3. منطق البحث الذكي بناءً على الصلاحيات
    if (searchQuery) {
      if (["ADMIN", "AGENT", "MERCHANT"].includes(currentUserRole)) {
        // الموظف يبحث في الكل (موظفين + عملاء)
        whereClause = {
          OR: [
            {
        ...
        // العميل يبحث فقط في طاقم العمل (إدارة/مندوب/تاجر)
        whereClause = {
          name: { contains: searchQuery, mode: "insensitive" },
          role: { in: ["ADMIN", "AGENT", "MERCHANT"] },
          isActive: true,
      ...
      console.log(
        `📝 [Contacts API] - ${currentUserRole} searching for: ${searchQuery}`,
      );
    }
    ...
    const contacts = await db.user.findMany({
      ...
      orderBy: { name: "asc" },
      take: searchQuery ? 30 : 100, // 👈 رفعنا العدد ليظهر كل الموظفين فوراً
    });
