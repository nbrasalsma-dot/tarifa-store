...
// 🛠️ دالة "العلاج الذكي": تقوم بإنشاء ملف متجر تلقائياً للتاجر إذا لم يكن لديه واحد
async function getOrHealMerchant(userId: string) {
  let merchant = await db.merchant.findFirst({
    where: { userId },
  });

  // إذا لم نجد المتجر، نقوم بالعلاج الذاتي وإنشائه فوراً
  if (!merchant) {
    ...
    if (user) {
      try {
        merchant = await db.merchant.create({
          data: {
            userId: user.id,
            storeName: user.name ? `متجر ${user.name}` : "متجر جديد",
            storeType: "عام",
            fullName: user.name || "تاجر",
            phone: user.phone || "000000000",
            email: user.email,
            address: user.address || "غير محدد",
            identityCardImage: "auto-healed-profile",
            isApproved: true, // تفعيل تلقائي لأن الإدارة أعطته الصلاحية
            isActive: true,
          },
        });
        console.log(
          "✅ [Auto-Heal] تم إنشاء ملف متجر تلقائياً للمستخدم:",
          userId,
        );
      } catch (e) {
        console.error("❌ [Auto-Heal] فشل في إنشاء ملف المتجر التلقائي:", e);
        return null;
      }
    }
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // جلب أو إنشاء بيانات التاجر بذكاء
    const merchant = await getOrHealMerchant(decoded.userId);

    if (!merchant) {
      return NextResponse.json(
        {
          success: false,
          error: "لم يتم العثور على بيانات التاجر وفشل الإصلاح التلقائي",
        },
        { status: 404 },
      );
    }
  ...
  try {
    ...
    const updatedProduct = await db.product.update({
      ...
      data: {
        ...
        estimatedDays: updateData.estimatedDays
          ? parseInt(updateData.estimatedDays)
          : null,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message:
        "تمت عملية الحذف بنجاح (تم مسح الصور وإخفاء المنتج نهائياً من المتجر)",
    });
  } catch (error) {
