...
export function NotificationBell({ userId }: { userId: string }) {
  ...
  const handleDeleteReview = async (
    ...
    if (!confirm("هل أنت متأكد من حذف هذا التقييم؟ سيختفي من الموقع نهائياً."))
      return;
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: "تم الحذف",
          description: "تمت إزالة التقييم المسيء بنجاح.",
        });
    ...
    >
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative hover:bg-[#FAF7F2] rounded-full h-10 w-10"
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-80 bg-white shadow-2xl border-t-4 border-[#C9A962] rounded-xl z-[60]"
      >
        <DropdownMenuLabel className="text-right font-bold text-[#3D3021] py-3">
          الإشعارات
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
              ...
              return (
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    ...
                    if (n.type === "NEW_MESSAGE" && extraData.conversationId) {
                      // 1. توجيه الرابط (يضيف رقم المحادثة في الرابط لتلتقطه واجهة الدردشة)
                      router.push(`/?chat=${extraData.conversationId}`);

                      // 2. إطلاق حدث برمجي (يفيد في فتح صندوق الرسائل فوراً إذا كان المستخدم في نفس الصفحة)
                      window.dispatchEvent(
                        new CustomEvent("open-chat", {
                          detail: extraData.conversationId,
                        }),
                      );
                    }
                    // إذا كان الإشعار يخص منتج (مثل تقييم أو استفسار)
                    else if (extraData.productId) {
                      router.push(`/products/${extraData.productId}`);
                    }
                  }}
                ...
                >
                  <div className="flex flex-col items-start text-right flex-1 ml-2">
                    <div className="flex items-center gap-1">
                      <span className="font-bold text-sm text-[#3D3021]">
                        {n.title}
                      </span>
                      {extraData.productId && (
                  ...
                  {n.type === "NEW_RATING" && extraData.reviewId && (
                    <Button
                      ...
                      onClick={(e) =>
                        handleDeleteReview(e, extraData.reviewId, n.id)
                      }
